...
"""Rpmfind: Search suggestions for Rpmfind."""
...
__prettyname__ = "Rpmfind: Search suggestions for Rpmfind"
__version__ = "0.1.0"
__trigger__ = "rpm "
...
icon_path = str(Path(__file__).parent / "search_rpmfind")
cache_path = Path(v0.cacheLocation()) / "search_rpmfind"
config_path = Path(v0.configLocation()) / "search_rpmfind"
data_path = Path(v0.dataLocation()) / "search_rpmfind"
        ...
        if not query.string:
            results.append(
                v0.Item(
                    id=__prettyname__,
                    icon=icon_path,
                    text=f'Search {"_".join("search_rpmfind".split("_")[1:])}',

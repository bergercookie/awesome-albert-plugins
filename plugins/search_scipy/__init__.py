...
"""Scipy: Search suggestions for Scipy."""
...
__prettyname__ = "Scipy: Search suggestions for Scipy"
__version__ = "0.1.0"
__trigger__ = "sp "
...
icon_path = str(Path(__file__).parent / "search_scipy")
cache_path = Path(v0.cacheLocation()) / "search_scipy"
config_path = Path(v0.configLocation()) / "search_scipy"
data_path = Path(v0.dataLocation()) / "search_scipy"
        ...
        if not query.string:
            results.append(
                v0.Item(
                    id=__prettyname__,
                    icon=icon_path,
                    text=f'Search {"_".join("search_scipy".split("_")[1:])}',

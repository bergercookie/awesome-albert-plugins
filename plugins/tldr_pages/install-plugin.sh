...
ret=$(is_installed tldr)
if [ "$ret" = "1" ]
then
    announce_err "Please install tldr first. Exiting"
    return 1
fi
...
PLUGIN_DIR="$DST/tldr_pages"
if [ -d "$PLUGIN_DIR" ]
then
    rm -rf "$PLUGIN_DIR"
fi

announce "Installing tldr_pages -> $PLUGIN_DIR"
cp -r "$(dirname "${BASH_SOURCE[0]}")" "$PLUGIN_DIR"
announce "Installed tldr_pages -> $PLUGIN_DIR"

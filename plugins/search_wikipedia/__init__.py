...
"""Wikipedia: Search suggestions for Wikipedia."""
...
__prettyname__ = "Wikipedia: Search suggestions for Wikipedia"
__version__ = "0.1.0"
__trigger__ = "w "
...
icon_path = str(Path(__file__).parent / "search_wikipedia")
cache_path = Path(v0.cacheLocation()) / "search_wikipedia"
config_path = Path(v0.configLocation()) / "search_wikipedia"
data_path = Path(v0.dataLocation()) / "search_wikipedia"
        ...
        if not query.string:
            results.append(
                v0.Item(
                    id=__prettyname__,
                    icon=icon_path,
                    text=f'Search {"_".join("search_wikipedia".split("_")[1:])}',

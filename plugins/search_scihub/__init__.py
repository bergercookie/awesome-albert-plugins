...
"""Scihub: Search suggestions for Scihub."""
...
__prettyname__ = "Scihub: Search suggestions for Scihub"
__version__ = "0.1.0"
__trigger__ = "sci "
...
icon_path = str(Path(__file__).parent / "search_scihub")
cache_path = Path(v0.cacheLocation()) / "search_scihub"
config_path = Path(v0.configLocation()) / "search_scihub"
data_path = Path(v0.dataLocation()) / "search_scihub"
        ...
        if not query.string:
            results.append(
                v0.Item(
                    id=__prettyname__,
                    icon=icon_path,
                    text=f'Search {"_".join("search_scihub".split("_")[1:])}',

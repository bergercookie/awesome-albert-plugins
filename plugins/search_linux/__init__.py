...
"""Linux: Search suggestions for Linux."""
...
__prettyname__ = "Linux: Search suggestions for Linux"
__version__ = "0.1.0"
__trigger__ = "linux "
...
icon_path = str(Path(__file__).parent / "search_linux")
cache_path = Path(v0.cacheLocation()) / "search_linux"
config_path = Path(v0.configLocation()) / "search_linux"
data_path = Path(v0.dataLocation()) / "search_linux"
        ...
        if not query.string:
            results.append(
                v0.Item(
                    id=__prettyname__,
                    icon=icon_path,
                    text=f'Search {"_".join("search_linux".split("_")[1:])}',

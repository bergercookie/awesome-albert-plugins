...
"""Gnu: Search suggestions for Gnu."""
...
__prettyname__ = "Gnu: Search suggestions for Gnu"
__version__ = "0.1.0"
__trigger__ = "gnu "
...
icon_path = str(Path(__file__).parent / "search_gnu")
cache_path = Path(v0.cacheLocation()) / "search_gnu"
config_path = Path(v0.configLocation()) / "search_gnu"
data_path = Path(v0.dataLocation()) / "search_gnu"
        ...
        if not query.string:
            results.append(
                v0.Item(
                    id=__prettyname__,
                    icon=icon_path,
                    text=f'Search {"_".join("search_gnu".split("_")[1:])}',

...
# Install ----------------------------------------------------------------------
install_pkg git+https://github.com/bergercookie/totp-cli

PLUGIN_DIR="$DST/pass_totp_cli"
if [ -d "$PLUGIN_DIR" ]
then
    rm -rf "$PLUGIN_DIR"
fi

announce "Installing pass_totp_cli -> $PLUGIN_DIR"
cp -r "$(dirname "${BASH_SOURCE[0]}")" "$PLUGIN_DIR"
announce "Installed pass_totp_cli -> $PLUGIN_DIR"

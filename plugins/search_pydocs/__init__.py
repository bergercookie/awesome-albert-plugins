...
"""Pydocs: Search suggestions for Pydocs."""
...
__prettyname__ = "Pydocs: Search suggestions for Pydocs"
__version__ = "0.1.0"
__trigger__ = "pydocs "
...
icon_path = str(Path(__file__).parent / "search_pydocs")
cache_path = Path(v0.cacheLocation()) / "search_pydocs"
config_path = Path(v0.configLocation()) / "search_pydocs"
data_path = Path(v0.dataLocation()) / "search_pydocs"
        ...
        if not query.string:
            results.append(
                v0.Item(
                    id=__prettyname__,
                    icon=icon_path,
                    text=f'Search {"_".join("search_pydocs".split("_")[1:])}',

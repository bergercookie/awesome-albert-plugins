...
"""Yahoo: Search suggestions for Yahoo."""
...
__prettyname__ = "Yahoo: Search suggestions for Yahoo"
__version__ = "0.1.0"
__trigger__ = "y "
...
icon_path = str(Path(__file__).parent / "search_yahoo")
cache_path = Path(v0.cacheLocation()) / "search_yahoo"
config_path = Path(v0.configLocation()) / "search_yahoo"
data_path = Path(v0.dataLocation()) / "search_yahoo"
        ...
        if not query.string:
            results.append(
                v0.Item(
                    id=__prettyname__,
                    icon=icon_path,
                    text=f'Search {"_".join("search_yahoo".split("_")[1:])}',

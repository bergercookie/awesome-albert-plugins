...
"""Cnn: Search suggestions for Cnn."""
...
__prettyname__ = "Cnn: Search suggestions for Cnn"
__version__ = "0.1.0"
__trigger__ = "cnn "
...
icon_path = str(Path(__file__).parent / "search_cnn")
cache_path = Path(v0.cacheLocation()) / "search_cnn"
config_path = Path(v0.configLocation()) / "search_cnn"
data_path = Path(v0.dataLocation()) / "search_cnn"
        ...
        if not query.string:
            results.append(
                v0.Item(
                    id=__prettyname__,
                    icon=icon_path,
                    text=f'Search {"_".join("search_cnn".split("_")[1:])}',

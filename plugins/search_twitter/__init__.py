...
"""Twitter: Search suggestions for Twitter."""
...
__prettyname__ = "Twitter: Search suggestions for Twitter"
__version__ = "0.1.0"
__trigger__ = "tw "
...
icon_path = str(Path(__file__).parent / "search_twitter")
cache_path = Path(v0.cacheLocation()) / "search_twitter"
config_path = Path(v0.configLocation()) / "search_twitter"
data_path = Path(v0.dataLocation()) / "search_twitter"
        ...
        if not query.string:
            results.append(
                v0.Item(
                    id=__prettyname__,
                    icon=icon_path,
                    text=f'Search {"_".join("search_twitter".split("_")[1:])}',

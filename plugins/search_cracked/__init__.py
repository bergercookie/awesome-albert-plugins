...
"""Cracked: Search suggestions for Cracked."""
...
__prettyname__ = "Cracked: Search suggestions for Cracked"
__version__ = "0.1.0"
__trigger__ = "cracked "
...
icon_path = str(Path(__file__).parent / "search_cracked")
cache_path = Path(v0.cacheLocation()) / "search_cracked"
config_path = Path(v0.configLocation()) / "search_cracked"
data_path = Path(v0.dataLocation()) / "search_cracked"
        ...
        if not query.string:
            results.append(
                v0.Item(
                    id=__prettyname__,
                    icon=icon_path,
                    text=f'Search {"_".join("search_cracked".split("_")[1:])}',

...
"""Imdb: Search suggestions for Imdb."""
...
__prettyname__ = "Imdb: Search suggestions for Imdb"
__version__ = "0.1.0"
__trigger__ = "imdb "
...
icon_path = str(Path(__file__).parent / "search_imdb")
cache_path = Path(v0.cacheLocation()) / "search_imdb"
config_path = Path(v0.configLocation()) / "search_imdb"
data_path = Path(v0.dataLocation()) / "search_imdb"
        ...
        if not query.string:
            results.append(
                v0.Item(
                    id=__prettyname__,
                    icon=icon_path,
                    text=f'Search {"_".join("search_imdb".split("_")[1:])}',

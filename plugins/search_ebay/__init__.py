...
"""Ebay: Search suggestions for Ebay."""
...
__prettyname__ = "Ebay: Search suggestions for Ebay"
__version__ = "0.1.0"
__trigger__ = "e "
...
icon_path = str(Path(__file__).parent / "search_ebay")
cache_path = Path(v0.cacheLocation()) / "search_ebay"
config_path = Path(v0.configLocation()) / "search_ebay"
data_path = Path(v0.dataLocation()) / "search_ebay"
        ...
        if not query.string:
            results.append(
                v0.Item(
                    id=__prettyname__,
                    icon=icon_path,
                    text=f'Search {"_".join("search_ebay".split("_")[1:])}',

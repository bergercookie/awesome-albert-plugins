...
"""Vim: Search suggestions for Vim."""
...
__prettyname__ = "Vim: Search suggestions for Vim"
__version__ = "0.1.0"
__trigger__ = "vim "
...
icon_path = str(Path(__file__).parent / "search_vim")
cache_path = Path(v0.cacheLocation()) / "search_vim"
config_path = Path(v0.configLocation()) / "search_vim"
data_path = Path(v0.dataLocation()) / "search_vim"
        ...
        if not query.string:
            results.append(
                v0.Item(
                    id=__prettyname__,
                    icon=icon_path,
                    text=f'Search {"_".join("search_vim".split("_")[1:])}',

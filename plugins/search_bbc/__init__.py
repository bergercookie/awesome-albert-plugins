...
"""Bbc: Search suggestions for Bbc."""
...
__prettyname__ = "Bbc: Search suggestions for Bbc"
__version__ = "0.1.0"
__trigger__ = "bbc "
...
icon_path = str(Path(__file__).parent / "search_bbc")
cache_path = Path(v0.cacheLocation()) / "search_bbc"
config_path = Path(v0.configLocation()) / "search_bbc"
data_path = Path(v0.dataLocation()) / "search_bbc"
        ...
        if not query.string:
            results.append(
                v0.Item(
                    id=__prettyname__,
                    icon=icon_path,
                    text=f'Search {"_".join("search_bbc".split("_")[1:])}',

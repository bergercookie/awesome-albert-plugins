...
"""Last: Search suggestions for Last."""
...
__prettyname__ = "Last: Search suggestions for Last"
__version__ = "0.1.0"
__trigger__ = "lfm "
...
icon_path = str(Path(__file__).parent / "search_last")
cache_path = Path(v0.cacheLocation()) / "search_last"
config_path = Path(v0.configLocation()) / "search_last"
data_path = Path(v0.dataLocation()) / "search_last"
        ...
        if not query.string:
            results.append(
                v0.Item(
                    id=__prettyname__,
                    icon=icon_path,
                    text=f'Search {"_".join("search_last".split("_")[1:])}',

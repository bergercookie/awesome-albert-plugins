...
"""Opencv: Search suggestions for Opencv."""
...
__prettyname__ = "Opencv: Search suggestions for Opencv"
__version__ = "0.1.0"
__trigger__ = "cv "
...
icon_path = str(Path(__file__).parent / "search_opencv")
cache_path = Path(v0.cacheLocation()) / "search_opencv"
config_path = Path(v0.configLocation()) / "search_opencv"
data_path = Path(v0.dataLocation()) / "search_opencv"
        ...
        if not query.string:
            results.append(
                v0.Item(
                    id=__prettyname__,
                    icon=icon_path,
                    text=f'Search {"_".join("search_opencv".split("_")[1:])}',

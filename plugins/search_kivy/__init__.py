...
"""Kivy: Search suggestions for Kivy."""
...
__prettyname__ = "Kivy: Search suggestions for Kivy"
__version__ = "0.1.0"
__trigger__ = "kv "
...
icon_path = str(Path(__file__).parent / "search_kivy")
cache_path = Path(v0.cacheLocation()) / "search_kivy"
config_path = Path(v0.configLocation()) / "search_kivy"
data_path = Path(v0.dataLocation()) / "search_kivy"
        ...
        if not query.string:
            results.append(
                v0.Item(
                    id=__prettyname__,
                    icon=icon_path,
                    text=f'Search {"_".join("search_kivy".split("_")[1:])}',
